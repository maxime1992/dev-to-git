...
import { DEV_TO_TOKEN } from './dev-to.tokens';
...
describe('DevTo interceptor', () => {
  ...
  describe('requestFulfilled', () => {
    ...
    it(`should not add any api-key header if the request isn't for dev.to API`, async () => {
      expect((await configCallback({baseURL:'http://test.com'})).headers?.['api-key']).toBeUndefined()
    });

    it(`should add an api-key header if the request is for dev.to API`, async () => {
      expect((await configCallback({baseURL:'https://dev.to/api'})).headers['api-key']).toBe(MOCK_DEV_TO_TOKEN)
      expect((await configCallback({baseURL:'https://dev.to/api/anything/else'})).headers['api-key']).toBe(MOCK_DEV_TO_TOKEN)
    });
  });
});
